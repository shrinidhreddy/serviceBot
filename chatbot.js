...
async function identify_user(agent)
{
  ...
  const snap = await client.db("serviceBot").collection("user_table").findOne({acct_num: acct_num});
  
  if(snap==null){
	  await agent.add("please enter valid number");
